...
import { 
  ...
  Users, Target, Award, Image as ImageIcon, Video as VideoIcon, AlignLeft
} from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { useBuilder } from '../context/BuilderContext';
import { aboutCategories, aboutLibrary, generateAboutCode } from '../data/aboutLibrary';
...
export default function AboutLibrary() {
  ...
  const [previewAbout, setPreviewAbout] = useState<any>(null);
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);

  const activeGroup = aboutLibrary.find(g => g.category === activeCategory);
  
  const handleCustomize = (about: any) => {
    setSelectedAbout(about);
    setCustomProps(about.defaultProps || {});
  };
  ...
  const handleCopyCode = (about: any) => {
    const code = generateAboutCode(about.componentName, customProps || about.defaultProps);
    navigator.clipboard.writeText(code);
    setCopiedId(about.id);
  ...
  const SidebarContent = () => (
    <div className="p-6">
      <h2 className="text-xs font-black text-white/40 uppercase tracking-widest mb-6 px-4">About Categories</h2>
      <div className="space-y-1">
        {aboutCategories.map(cat => {
          ...
          return (
            <button
              key={cat}
              onClick={() => { setActiveCategory(cat); setSelectedAbout(null); setMobileMenuOpen(false); }}
    ...
    <div className="flex flex-col md:flex-row h-[calc(100vh-64px)] bg-[#09090b] overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-10 relative custom-scrollbar bg-[#0d0d0f]">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 md:mb-10 gap-4">
          <div>
            <h1 className="text-3xl md:text-5xl font-black text-white tracking-tighter mb-2 uppercase">{activeCategory} About</h1>
            <p className="text-white/40 font-medium text-sm md:text-base text-balance">Tell your brand's unique story with high-impact visuals.</p>
          </div>
        ...
        <div className={viewMode === 'grid' ? "grid grid-cols-1 xl:grid-cols-2 gap-6 md:gap-8" : "flex flex-col gap-6 md:gap-8"}>
          {activeGroup?.items.map((about) => {
            const ComponentRenderer = (allAbout as any)[about.componentName];
            const currentProps = (selectedAbout?.id === about.id) ? customProps : about.defaultProps;

            return (
              <motion.div key={about.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                className={`bg-[#18181b] rounded-3xl border ${selectedAbout?.id === about.id ? "border-indigo-500 shadow-indigo-500/20" : "border-white/5"} overflow-hidden group shadow-2xl flex flex-col transition-all duration-300`}>
                ...
                <div className="relative group/preview inset-0 overflow-hidden rounded-t-[2.5rem]">
                  <PreviewContainer height={600} width={1440}>
                    {ComponentRenderer ? <ComponentRenderer {...currentProps} isPreview={true} /> : <div className="p-20 text-white text-4xl font-black">PREVIEW</div>}
                </PreviewContainer>
                  ...
                  <div className="absolute bottom-4 right-4 opacity-0 group-hover/preview:opacity-100 flex gap-2 transition-opacity duration-300 z-50">
                     <button onClick={(e) => { e.stopPropagation(); handleCustomize(about); }} className="bg-indigo-600/90 text-white px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 hover:bg-indigo-500 shadow-xl backdrop-blur-md">
                         <SlidersHorizontal size={14}/> Customize
                     </button>
                     <button onClick={(e) => { e.stopPropagation(); handleCopyCode(about); }} className="bg-[#18181b]/90 border border-white/10 text-white px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 hover:bg-[#1f1f23]/90 shadow-xl backdrop-blur-md">
                ...
                <div className="p-6 md:p-8 flex-1 flex flex-col justify-between">
                  <div>
                    <span className="text-[10px] font-black uppercase text-indigo-400 tracking-widest bg-indigo-500/10 px-2 py-1 rounded">{about.tag}</span>
                    <h3 className="text-xl md:text-2xl font-bold text-white mt-4 mb-2 tracking-tight">{about.name}</h3>
                    <p className="text-xs md:text-sm text-white/40 leading-relaxed font-medium line-clamp-2">{about.desc}</p>
                  </div>
                  
                  <div className="mt-6 flex flex-col sm:flex-row gap-3">
                     <button onClick={() => setPreviewAbout(about)} className="flex-1 bg-white/5 hover:bg-white/10 text-white font-black text-[10px] uppercase tracking-[0.2em] py-4 rounded-xl border border-white/5 flex justify-center items-center gap-2 transition active:scale-95 group-hover:border-indigo-500/30">
                        <Eye size={16}/> VIEW
                     </button>
                     <button onClick={() => handleUseThis(about.id, about.componentName)} className="flex-[1.5] bg-indigo-600 hover:bg-indigo-500 text-white font-black text-[10px] uppercase tracking-[0.2em] py-4 rounded-xl flex justify-center items-center gap-2 shadow-xl shadow-indigo-600/20 transition active:scale-95">
          ...
          {activeGroup?.items.length === 0 && (
            <div className="col-span-full py-20 text-center border-2 border-dashed border-white/5 rounded-[3rem]">
               ...
               <p className="text-white/20 font-black uppercase tracking-widest">More layouts coming soon</p>
            </div>
          )}
        </div>
      </div>
          ...
          <motion.div initial={{ width: 0, opacity: 0 }} animate={{ width: 350, opacity: 1 }} exit={{ width: 0, opacity: 0 }}
            ...
            <div className="p-6 border-b border-white/5 sticky top-0 bg-[#111113]/90 backdrop-blur z-10 flex justify-between items-center">
              <div>
                ...
                <p className="text-[10px] text-white/40 mt-1 uppercase font-black tracking-widest">{selectedAbout.name}</p>
              </div>
              <button onClick={() => setSelectedAbout(null)} className="p-2 bg-white/5 hover:bg-white/10 rounded-full text-white/50"><X size={16}/></button>
            </div>
            ...
            <div className="p-6 space-y-10 pb-32">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="text-[10px] font-black uppercase text-white/20 mb-2 block">Accent Color</label>
                    <div className="flex gap-2">
                       <input type="color" value={customProps.accentColor || "#6366f1"} onChange={(e) => updateProp('accentColor', e.target.value)} className="w-10 h-10 rounded-lg cursor-pointer bg-transparent border-0" />
                       <input type="text" value={customProps.accentColor || ""} onChange={(e) => updateProp('accentColor', e.target.value)} className="flex-1 bg-[#09090b] border border-white/10 rounded-lg px-2 text-[10px] text-white font-mono uppercase" />
                    </div>
                  </div>
                </div>
              </div>
              ...
              {/* MEDIA */}
              <div className="space-y-6 pt-6 border-t border-white/5">
                 <h4 className="text-[10px] font-black text-white/30 uppercase tracking-[0.2em] flex items-center gap-2"><ImageIcon size={14}/> Media Assets</h4>
                 <div className="space-y-4">
                    <button className="w-full py-4 border-2 border-dashed border-white/5 rounded-2xl text-white/20 hover:text-white/40 hover:border-white/10 transition-all flex flex-col items-center gap-2">
                       <Users size={24}/>
                       <span className="text-[10px] font-black uppercase tracking-widest">Update Team Photos</span>
                    </button>
                    <button className="w-full py-4 border-2 border-dashed border-white/5 rounded-2xl text-white/20 hover:text-white/40 hover:border-white/10 transition-all flex flex-col items-center gap-2">
                       <VideoIcon size={24}/>
                       <span className="text-[10px] font-black uppercase tracking-widest">Update About Video</span>
                    </button>
                 </div>
              </div>
              ...
              {/* PRESETS */}
              <div className="space-y-6 pt-6 border-t border-white/5">
                <h4 className="text-[10px] font-black text-white/30 uppercase tracking-[0.2em] flex items-center gap-2"><Zap size={14}/> Style Presets</h4>
                <div className="grid grid-cols-2 gap-2">
                   <button onClick={() => setCustomProps({...customProps, bgColor: "#09090b", textColor: "#ffffff", accentColor: "#6366f1"})} className="py-3 rounded-xl bg-black border border-white/10 text-white text-[10px] font-bold uppercase">Modern Dark</button>
                   <button onClick={() => setCustomProps({...customProps, bgColor: "#ffffff", textColor: "#0f172a", accentColor: "#4f46e5"})} className="py-3 rounded-xl bg-white border border-black/10 text-black text-[10px] font-bold uppercase">Soft Light</button>
                </div>
              </div>
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 30 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 30 }}
              ...
              <div className="p-6 md:p-10 flex items-center justify-between border-b border-white/5 bg-[#111] z-10 shrink-0">
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    <h2 className="text-2xl md:text-3xl font-black text-white tracking-tighter uppercase leading-none mb-2">{previewAbout.name}</h2>
                    <p className="text-white/40 text-[10px] font-black uppercase tracking-[0.3em]">{previewAbout.tag}</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <button onClick={() => handleUseThis(previewAbout.id, previewAbout.componentName, customProps || previewAbout.defaultProps)} className="hidden md:flex bg-indigo-600 hover:bg-indigo-500 text-white px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] items-center gap-3 transition-all active:scale-95 shadow-xl shadow-indigo-600/20">
                    <Zap size={18} fill="currentColor" /> Add to Builder
                  </button>
                  <button onClick={() => setPreviewAbout(null)} className="p-4 bg-white/5 hover:bg-white/10 text-white/50 hover:text-white rounded-2xl transition-all border border-white/5 shadow-xl">
                    <X size={24} />
                  </button>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-auto bg-[radial-gradient(circle_at_50%_50%,rgba(99,102,241,0.05),transparent)] flex items-start justify-center p-4 md:p-12 relative">
                <div className="w-full relative shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] border border-white/5 rounded-2xl overflow-hidden bg-[#0a0a0c]">
                  {(() => {
                    const ComponentRenderer = (allAbout as any)[previewAbout.componentName];
                    const props = (selectedAbout?.id === previewAbout.id) ? customProps : previewAbout.defaultProps;
              ...
              <div className="md:hidden p-6 border-t border-white/5 bg-[#111] shrink-0">
                 <button onClick={() => handleUseThis(previewAbout.id, previewAbout.componentName, customProps || previewAbout.defaultProps)} className="w-full bg-indigo-600 text-white py-5 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] flex items-center justify-center gap-3 active:scale-95">
                    <Zap size={18} fill="currentColor" /> Add to Builder
                 </button>
              </div>
            </motion.div>
